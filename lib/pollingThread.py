...
class PollingThread:
	...
	def treatTaskResult(self, task, taskResultFilePath):
		...
		key = 'secret#456!23key'
		iv = 'Key@123Key@123fd'
		enc_cmdresult = self.dropboxHandler.readFile(taskResultFilePath, resultFormat="string")
		...
		if proceed:
			if cmd in ['runCLI', 'runPSModule']:
				...
				with open(localFile, 'w+') as fileHandle:
					fileHandle.write(self.dropboxHandler.readFile(result, resultFormat="string"))
					fileHandle.close()
	...
	def treatPushedData(self, pushedDataFilePath):
		...
		key = 'secret#456!23key'
		iv = 'Key@123Key@123fd'
		enc_cmdresult = self.dropboxHandler.readFile(pushedDataFilePath, resultFormat="string")
		decodetext =  base64.b64decode(enc_cmdresult)
		aes = AES.new(key, AES.MODE_CBC, iv)
		encoder = PKCS7Encoder()
		cipher = aes.decrypt(decodetext)
